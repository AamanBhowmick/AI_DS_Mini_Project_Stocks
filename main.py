...
try:
    ...
    if st.button("Analyze"):
            ...
            data_ma = pd.DataFrame({'Moving Average':df['Adj Close'].rolling(1).mean(), 'MA 100':df['Adj Close'].rolling(100).mean(), 'MA 200':df['Adj Close'].rolling(200).mean()})
            st.markdown('MA COMPARISION')
            st.line_chart(data_ma)
            ...
            with st.spinner('(MIGHT TAKE 15-20 SECONDS)'):
            ...
            st.checkbox("TRAINING THE MODEL", value=True, disabled=True)
